...
function dialogForRecord(record){const id=String(record?.id||'').trim();return[...document.querySelectorAll('dialog.client-dialog')].reverse().find(d=>d.isConnected&&states.has(d)&&(id?d.dataset.clientId===id:(d.dataset.newClient==='true'&&!d.dataset.clientId)))||null;}
const baseSave=mhRepository.saveClient.bind(mhRepository);
mhRepository.saveClient=async function saveClientWithNotesExtras(record,...args){const dialog=dialogForRecord(record);const saved=await baseSave(record,...args);if(saved?.id&&dialog){await flushQueued(dialog,saved.id);const host=dialog.querySelector('[data-notes-extras-documents]');if(host){host.remove();bind(dialog);}}return saved;};
