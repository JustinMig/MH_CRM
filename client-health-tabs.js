...
function refreshEmpty(panel, kind) {
  const list = panel.querySelector(`[data-health-list="${kind}"]`);
  const empty = panel.querySelector(`[data-health-empty="${kind}"]`);
...
function bind(form) {
  ...
  form.addEventListener('click', event => {
    ...
    if (add) {
      ...
      newCard.open = true;
      refreshEmpty(panel, kind);
      form.dispatchEvent(new Event('input',{bubbles:true}));
      newCard.querySelector('input,select,textarea')?.focus();
      return;
    }
    const remove = event.target.closest?.('[data-remove-health-record]');
    if (remove) {
      const card = remove.closest('[data-health-card]');
      const panel = card?.closest('[data-panel]');
      const kind = panel?.dataset.panel;
      card?.remove();
      if (panel && kind) refreshEmpty(panel, kind);
      form.dispatchEvent(new Event('input',{bubbles:true}));
    }
  });
  form.addEventListener('input', event => {
    const card = event.target.closest?.('[data-health-card]');
    if (!card) return;
...
function parseRows(record, prefix, keys) {
  ...
  return [...indexes].sort((a,b)=>a-b).map(i => {
    const out = {};
    keys.forEach(key => out[key] = record[`${prefix}_${i}_${key}`] ?? '');
    return out;
  });
}
...
async function syncTable(table, clientId, rows, mapper) {
  ...
  for (const row of rows) {
    ...
    if (row.id) {
      const { data, error } = await supabase.from(table).update(payload).eq('id',row.id).eq('client_id',clientId).select('id').maybeSingle();
      if (error) throw error;
      if (data?.id) keep.push(data.id);
    } else {
...
mhRepository.saveClient = async function saveClientWithHealthTabs(record, ...args) {
  const doctors = parseRows(record,'doctor',doctorKeys);
  const medications = parseRows(record,'medication',medicationKeys);
  const indemnity = parseRows(record,'indemnity',indemnityKeys);
...
Dialogs.prototype.open = function patchedHealthTabsOpen(options = {}) {
  ...
  controller.attachForm = form => {
    const extra = clientId ? (cache.get(clientId) || {}) : {};
    render(form, extra);
    previousAttach(form);
  };
  return controller;
