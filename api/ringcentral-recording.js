...
export async function GET(request) {
  try {
    ...
    const rows = await adminRest(`/rest/v1/ringcentral_calls?recording_id=eq.${encodeURIComponent(recordingId)}&select=id,recording_id&limit=1`);
    if (!Array.isArray(rows) || !rows[0]) return Response.json({ error: 'Recording is not linked to an M&H client.' }, { status: 404 });
