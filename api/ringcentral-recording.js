import { adminRest, requireCrmUser, getClient } from '../server/communications.js';
...
export async function GET(request) {
  try {
    ...
    const rows = await adminRest(`/rest/v1/ringcentral_calls?recording_id=eq.${encodeURIComponent(recordingId)}&hidden_at=is.null&select=id,client_id,recording_id&limit=1`);
    if (!Array.isArray(rows) || !rows[0]) return Response.json({ error: 'Recording is not linked to a visible M&H client call.' }, { status: 404 });

    if (!await getClient(rows[0].client_id, user)) return Response.json({ error:'Recording is not available.' },{status:404});
